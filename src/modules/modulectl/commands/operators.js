...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName("operator")
    .setDescription("Add/remove an operator for a module")
    ...
    .addStringOption((option) =>
      option
        ...
        .setChoices(moduleNames.map((m) => ({ name: m.name, value: m.name })))
        .setRequired(true)
    )
  ...
  async execute(client, interaction) {
    ...
    if (
      !checkOperatorship(interaction.member, interaction.guildId, "operators")
    ) {
    ...
    const roleExists = rules.existsInTable(
      role.id,
      moduleName,
      interaction.guildId,
      MODULE_SETTING.OPERATORS
    );
    let logMessage = ``;
    if (action === "add") {
      if (roleExists) {
        await interaction.editReply({
          content: "This role already is an operator for the module",
        });
        return;
      }
      access.add(
    ...
    if (action === "remove") {
      if (!roleExists) {
        await interaction.editReply({
          content: "This role already isn't an operator for the module",
        });
        return;
      }
